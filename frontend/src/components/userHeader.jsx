...
import { removeToken, getUser, authFetch } from '../utils/auth';

export default function UserHeader({ page, userData }) {
  ...
  const navItems = [
    { key: "./user", label: "User", icon: "dashboard" },
    { key: "./user/projects", label: "Projects", icon: "assignment" },
    { key: "./user/alerts", label: "Alerts", icon: "notifications" },
  ...
  // Fetch alert count for notification badge (only undismissed alerts)
  useEffect(() => {
    const user = getUser();
    if (!user) return;
      ...
      <header className="w-full sticky top-0 z-40 bg-surface dark:bg-surface-dim shadow-sm flex justify-between items-center px-6 py-md md:px-margin-desktop">
        ...
        <nav className="hidden md:flex items-center space-x-8">
          {navItems.map((item) => (
            ...
            >
              ...
              {item.label}
              {item.key === "./user/alerts" && alertCount > 0 && (
                <span className="absolute -top-1 -right-0.5 bg-red-500 text-white text-[10px] font-bold rounded-full w-5 h-5 flex items-center justify-center">
        ...
        <div className="flex items-center gap-4 relative">
          ...
          <div className="flex flex-col items-end mr-2">
            <span className="text-sm font-medium text-on-surface">{userData?.username || 'User'}</span>
            ...
            {userData?.userImage ? (
              <img
                ...
                onError={(e) => {
                  ...
                  e.target.src = 'https://ui-avatars.com/api/?name=' + (userData?.username || 'User') + '&background=6366f1&color=fff&size=40';
                }}
              />
            ) : (
